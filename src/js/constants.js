/**
 * for ScriptCreation.vue, set the number of rows when first entry
 */
export const DEFAULT_ROW_NUM = 8
/**
 * The directory of electron files in appData
 */
export const ELECTRON_DIR = 'resource'
/**
 * The path where configurations are stored
 */
export const DEFAULT_CONFIG_DEST = `${ELECTRON_DIR}/config.json`
/**
 * The path where the database is
 */
export const DEFAULT_BD_DEST = `${ELECTRON_DIR}/vpro.db`
/**
 * The path where the files of system are.
 * Files such as audio wave canvas images, garbage files
 */
export const DEFAULT_SYS_DIR = `${ELECTRON_DIR}/sys-file/`
/**
 * The directories for system
 */
export const DIR_FOR_SYS = {
  WAVEFORM: 'waveform',
  GARBAGE: 'garbage'
}
/**
 * The field names of configuration json file
 */
export const CONFIG_FIELD_NAMES = {
  /**
   * The last picked path for choosing the current folder with dialog
   */
  LAST_PICKED_PATH: 'lastPickedPath',
  /**
   * The last used path(current folder)
   */
  LAST_USED_PATH: 'lastUsedPath',
  /**
   * The last selected index of the file list on the left side of file explorer
   */
  LAST_SELECTED_INDEX: 'lastSelectedIndex',
  /**
   * The dir where stores the file generated by this app.
   * Files such as audio wave canvas images, garbage files
   */
  SYS_FILES_DIR: 'sysFilesDir',
  /**
   * The last used media type for storaging while browsing
   */
  LAST_MEDIA_TYPE: 'lastMediaType',
}

/**
 * The titles of a video script
 */
export const TITLES = ['大纲', '分镜', '文件路径', '台词文案', '备注', '创建时间', '修改时间']
/**
 * The bounds of window
 */
export const BOUNDS = {
  WIDTH: 1600,
  HEIGHT: 1200,
  MAX_WIDTH: 2000,
  MIN_WIDTH: 1200,
  MAX_HEIGHT: 1200,
  MIN_HEIGHT: 1200,
}

/**
 * Used for the communication within vues via ipc
 */
export const SPEC = {
  vueNames: {
    SCRIPT_CREATION: 'ScriptCreation',
    FILE_EXPLORER: 'FileExplorer',
    PAGINATION: 'Pagination',
    DRAW_WAVE: 'DrawWave',
    SITE_BROWSER: 'SiteBrowser',
  },
  type: {
    FILE_SHARE: 'fileShare',
    PARENT_TO_CHILD: 'parentToChild',
    REFRESH_PAGED_FILE_PATHS: 'refreshPagedFilePaths',
    RETRIEVE_CANVAS: 'retrieveCanvas',
    PASS_MEDIA_INFO_TO_MAIN_PROCESS: 'passMediaInfoToMainProcess',
  }
}

/**
 * database operation type
 */
export const OPERATION_TYPE = {
  DELETION: 'deletion',
  SELECTION: 'selection',
  UPDATE: 'update',
  INSERTION: 'insertion',
}

/**
 * a map where key is alias, value is the table name in db
 */
export const tableNameMap = {
  scriptRows: 't_script_row',
  video: 't_video',
  scriptBk: 't_script_bk',
  fileRank: 't_file_rank',
  globalShortcut: 't_global_shortcut',
  audioWaveCanvas: 't_audio_wave_canvas',
}

/**
 * to mark status of something
 */
export const STATUS = {
  ORDINARY: '0',
  DELETED: '1',
}

/**
 * statistic for  all transactional operations
 */
export const TRANSACTIONS = {
  SAVE_SCRIPT_ROWS: 'saveScriptRows',
  RECREATE_SCRIPT_BK: 'recreateScriptBk',
  INSERT_SCRIPT_BK_AFTER_DELETION: 'insertScriptBkAfterDeletion',
}

/**
 * File extensions, currently audio, video, image 
 */
export const EXTENSIONS = {
  // audition suffixes
  AUDIO: [".mp3", ".wav", ".flac", ".aac", ".ogg", ".wma", ".m4a"],
  // video suffixes
  VIDEO: [".mp4", ".avi", ".mkv", ".mov", ".wmv", ".flv", ".webm", ".m4v"],
  // image suffixes
  IMAGE: [".jpg", ".jpeg", ".png", ".gif", ".bmp", ".tiff", ".tif", ".webp", ".svg", ".ico"]
}

/**
 * hrefs of routers
 */
export const ROUTER_HREF = {
  Home: '#/home',
  FileExplorer: '#/fileExplorer',
  VideoSelection: '#/videoSelection',
  SiteBrowser: '#/siteBrowser',
}

/**
 * Array consists of hrefs that need to unmount components via ipc
 */
export const HREFS_NTUC = [
  ROUTER_HREF.FileExplorer
]